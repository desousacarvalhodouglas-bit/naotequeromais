...
import { Heart, Share2, MessageSquare, MapPin, X, Camera, Globe, Users, Send, Video } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { mockPosts, mockThemes } from '../mock/data';
...
const PostCard = ({ post, onRecommend, onRespond }) => {
  ...
  return (
    <Card className="p-4 mb-3 hover:shadow-md transition-shadow border border-gray-100">
...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FFF5F3] pb-16 md:pb-0">
      ...
      {recommendedToast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 bg-blue-600 text-white px-6 py-3 rounded-lg shadow-lg text-sm animate-in fade-in slide-in-from-top-2">
          {recommendedToast}
        </div>
      )}
      ...
      {showBanner && (
        <div className="bg-[#FFE5E0]">
          <div className="max-w-7xl mx-auto px-4 py-3 text-center relative">
            ...
            <Button size="sm" className="bg-[#FF6B6B] hover:bg-[#E85D4A] text-white mt-2 rounded-full px-6" onClick={() => navigate('/abonamento')}>
              Assinar novamente
            </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="mt-6 mb-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                {mockThemes.map((theme) => (
                  <div key={theme.id} className="relative rounded-xl overflow-hidden cursor-pointer group h-48">
          ...
          <div className="space-y-4">
            ...
            <Card className="p-4">
              ...
              <div className="mt-3">
                  ...
                  {[0, 1, 2].map((i) => (
                    <div key={i} className="relative">
                      {postPhotos[i] ? (
                        ...
                        <label className="w-16 h-16 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center cursor-pointer hover:border-green-400 transition-colors">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => cameraInputRef.current?.click()} className="flex items-center gap-1 text-xs text-green-600 bg-green-50 px-3 py-1.5 rounded-full hover:bg-green-100 transition-colors">
                    <Camera className="w-3.5 h-3.5" /> Tirar foto
                  </button>
                  <button onClick={() => videoInputRef.current?.click()} className="flex items-center gap-1 text-xs text-blue-600 bg-blue-50 px-3 py-1.5 rounded-full hover:bg-blue-100 transition-colors">
            ...
            <Card className="p-4">
              ...
              <Button className="w-full bg-[#FF9B8A] hover:bg-[#FF8A79] text-white rounded-full h-11" onClick={() => navigate('/empregos')}>
